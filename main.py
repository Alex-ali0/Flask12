...
facts =['Большинство людей, страдающих технологической зависимостью, испытывают сильный стресс, когда они находятся вне зоны покрытия сети или не могут использовать свои устройства.',
        ...
        'Изучение технологической зависимости является одной из наиболее актуальных областей научных исследований в настоящее время.<a href="/secret">секретная страница!</a>',
...
@app.route("/")
def home():
    return'<p>Добро пожаловать на сайт о технологических зависимостях</p> <a href="/facts">Посмотреть случайный факт!</a>\n<a href="/password">сгенерировать случайный пароль</a>'

# ссылка на секретную странницу находится в одном из случайных фактов
@app.route("/secret")
def secrets():
    return f"<p>Вы нашли секретную странницу</p> <a href='/game'>играть в орёл и решка</a>"

@app.route("/game")
def game():
    rand_num = [1,2]
    rand_str_orel = ['орел','ОРЕЛ','Орёл']
    rand_str_reshka = ['решка','РЕШКА','Решка']
    if random.choice(rand_num)==1:
        return random.choice(rand_str_orel)
    else:
        return random.choice(rand_str_reshka)
